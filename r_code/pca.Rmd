...
```{r}
method_sub <- para_sub %>% filter(method == 'i2')

pca1 <- prcomp(method_sub[,c(15:18)],center = FALSE, scale. = FALSE)
...
```{r}
df1$name <- method_sub$name
df1$weight <- method_sub$weight
df1$class <- method_sub$class
...
```{r}
class_sub <- method_sub %>% filter(class == 1 & nclass == 'c4')
list(class_sub$lnl)
```
